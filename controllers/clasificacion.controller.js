...
const GetAllClasificaciones = (req, res) => {
    const query = `
        SELECT c.*,
               (c.goles_a_favor - c.goles_en_contra) AS diferencia_goles,
               e.nombre_equipo,
        ...
        ORDER BY c.id_division, c.puntos DESC, (c.goles_a_favor - c.goles_en_contra) DESC
    `;
...
const GetClasificacionByDivision = (req, res) => {
    ...
    const query = `
        ...
        WHERE c.id_division = ?
          AND e.activo_equipo = 1
          AND d.activo_division = 1
        ORDER BY c.puntos DESC, (c.goles_a_favor - c.goles_en_contra) DESC
    `;
...
const UpdateClasificacion = (req, res) => {
    const { id } = req.params;
    const { id_equipo, id_division, puntos, partidos_jugados, partidos_ganados, partidos_empatados, partidos_perdidos, goles_a_favor, goles_en_contra } = req.body;
    const query = 'UPDATE clasificacion SET id_equipo = ?, id_division = ?, puntos = ?, partidos_jugados = ?, partidos_ganados = ?, partidos_empatados = ?, partidos_perdidos = ?, goles_a_favor = ?, goles_en_contra = ? WHERE id_clasificacion = ?';
    connection.query(query, [id_equipo, id_division, puntos, partidos_jugados, partidos_ganados, partidos_empatados, partidos_perdidos, goles_a_favor, goles_en_contra, id], (error, results) => {
        if (error) {
